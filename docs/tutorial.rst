...
It's a valkka_multiprocess module that servers as a template for python3 projects:
...
.. code:: text
    ...
    valkka_multiprocess/               # python package dir
        ...
        valkka_multiprocess/           # python source code directory
            ...
            valkka_multiprocess_cpp/   # interfaced cpp code as this python package's submodule OR as a separate python package
...
.. code:: text
    ...
    valkka_multiprocess/
        ...
        valkka_multiprocess/               # THE ORIGINAL PYTHON PACKAGE DIRECTORY (see above)
            bash/               # helper scripts
...
.. code:: bash
    ...
    git clone https://github.com/elsampsa/valkka_multiprocess
    mv valkka_multiprocess your_package_name
...
That should change all occurrences of "valkka_multiprocess" (in the file and directory names and inside the files)
...
.. code:: text
    ...
    /usr/src/app/          # directory on the docker image
        ...
        valkka_multiprocess/          # python SOURCE CODE DIRECTORY as a shared docker mount
...
This page has been produced with the file ``valkka_multiprocess/docs/tutorial.rst``.  Go ahead and open it in your editor.  Open also "index.rst".  Edit them accordingly.  
Said that, let's take a closer look at the directory structure (in the following ``valkka_multiprocess`` is synonymous to ``your_package_name``):

* ``valkka_multiprocess/`` : This is the python package directory
...
* ``valkka_multiprocess/sudo`` : System-specific install scripts
...
* ``valkka_multiprocess/docs/``  : Documentation and autogenerated documentation lives here
...
* ``valkka_multiprocess/valkka_multiprocess`` : Python source code directory
...
Now in the upper level directory you will have the file *valkka_multiprocess_0.0.0-0ubuntu1-bionicppa1_source.changes*.  You can send it to your Ubuntu PPA (personal package archive) with:

.. code:: bash

    dput ppa:your-name/your-repo valkka_multiprocess_0.0.0-0ubuntu1-bionicppa1_source.changes
